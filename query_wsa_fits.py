...
def query_wsa_fits(ra,dec,size=10.0,catalog='UKIDSS',output_file='',filter='all'):
	...
	if catalog=='UHS':
		curl = 'curl -X POST -d "ra='+str(ra)+'&dec='+str(dec)+'&filterID='+filter+'&sys=J2000&xsize='+str(size)+'&ysize='+str(size)+'&obsType=object&frameType=stack&database=UHSDR1&programmeID=all&submit=Submit" http://wsa.roe.ac.uk:8080/wsa/GetImage'
