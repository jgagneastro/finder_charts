...
def query_wsa_fits(ra,dec,size=10.0,catalog='UKIDSS',output_file='',filter='all'):
	...
	if catalog=='VHS':
	    curl = 'curl -X POST -d "database=VHSDR4&programmeID=110&ra='+str(ra)+'&dec='+str(dec)+'&sys=J&filterID='+filter+'&xsize='+str(size)+'&ysize='+str(size)+'&obsType=object&frameType=tilestack&mfid=&fsid=&archive=VSA" http://horus.roe.ac.uk:8080/vdfs/GetImage'
#   if output_file=='':
